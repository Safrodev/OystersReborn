...
public class OysterBlock extends BlockWithEntity implements Waterloggable {
    ...
    private final VoxelShape voxelShape = Block.createCuboidShape(3.0D, 0.0D, 3.0D, 13.0D, 8.0D, 13.0D);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new OysterEntity(pos, state);
    }

    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, OystersManager.OYSTER_BLOCK_ENTITY, OysterEntity::tick);
    }
}
